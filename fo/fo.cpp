...
static void PrintVcproj(const TSourceFolder &proj, const TString &vcProjFile, const TString &pathToSrc, const TString &utilPath)
{
    ...
    if (proj.IsUsingCuda) {
        ...
        ext.Add(TXmlNode("Import").Prop("Project", "$(VCTargetsPath)\\BuildCustomizations\\CUDA 12.3.props"));
        root.Add(ext);
    }
    ...
    if (proj.IsUsingCuda) {
        ...
        cudaTargets.Add(TXmlNode("Import").Prop("Project", "$(VCTargetsPath)\\BuildCustomizations\\CUDA 12.3.targets"));
