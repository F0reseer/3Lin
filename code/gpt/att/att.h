...
void GroupAttention(const TAttentionInfo &attSrc, TAttentionInfoGrouped<N> *p)
{
    ...
    for (yint g = 0; g < groupCount; ++g) {
        ...
        for (;;) {
            ...
            for (yint k = 0; k < N; ++k) {
                gg.Spans[k].Start = FAR_RIGHT;
                gg.Spans[k].Finish = FAR_RIGHT - 1;
            }
            ...
            for (;;) {
                ...
                for (yint k = 0; k < N; ++k) {
                    if (!taken[k] && ptrArr[k] != ptrFinArr[k]) {
                        const TAttentionSpan &span = att.Spans[ptrArr[k]];
                        if (span.Start / N >= start / N || span.Start / N <= finish / N) {
            ...
            gg.Start = (gg.Start / N) * N;
            gg.Finish = (gg.Finish / N) * N;
            ggArr.push_back(gg);
        }
