...
{
    QK = CreateModelMatrix(cpuMatrixAdd, matrixScale, DISCR_SCALE, dim, qDim, MM_DISP_MATRIX, quant, MM_SYNC_GRADIENT);
    QV = CreateModelMatrix(cpuMatrixAdd, matrixScale, DISCR_SCALE, dim, qDim, MM_DISP_MATRIX, quant, MM_SYNC_GRADIENT);
    K = CreateModelMatrix(cpuMatrixAdd, matrixScale, DISCR_SCALE, dim, ttDim, MM_DISP_MATRIX, quant, MM_SYNC_GRADIENT);
    V = CreateModelMatrix(cpuMatrixAdd, matrixScale, DISCR_SCALE, dim, ttDim, MM_DISP_MATRIX, quant, MM_SYNC_GRADIENT);
    Combiner = CreateModelMatrix(cpuMatrixAdd, matrixScale, DISCR_SCALE, GetCombinerWidth(ttDim), dim, MM_DISP_MATRIX, quant, MM_SYNC_GRADIENT);
    AlibiSlope = layerParams.AlibiSlope;
    AlibiHyper = layerParams.AlibiHyper;
    WideLayer = layerParams.WideLayer;
}
...
void TAttentionParams::GetParams(TModelParams::TAttentionMatrices *p, TModelDim::TAttentionPosParams *pLayerParams)
{
    ...
    pLayerParams->WideLayer = WideLayer;
}
...
class TModel : public IModel
{
    ...
    void Allocate(yint deviceCount, const TModelDim &md)
    {
        ...
        EModelMatrixQuant quant = ModelDim.HasFlag(MPF_SIM_QUANT_2BIT) ? MM_QUANT_2BIT : MM_QUANT_NONE;

        MatrixScale = new TModelMatrixScale(maxMatrixCount);
        MatrixAdd = new TCPUMatrixAdd(deviceCount, maxMatrixCount, DeltaHookGen.Get());
        FinalLayer = CreateModelMatrix(MatrixAdd, MatrixScale, DISCR_SCALE, md.Dim, md.VocabSize, MM_DISP_ROW, MM_QUANT_NONE, MM_SYNC_GRADIENT);
        LabelEmbed = CreateModelMatrix(MatrixAdd, MatrixScale, DISCR_SCALE, md.Dim, md.LabelCount, MM_DISP_ROW, MM_QUANT_NONE, MM_STALE_GRADIENT);
...
public:
    ...
    {
        p->ModelDim = ModelDim;
        LabelEmbed->GetDeltaData(&p->LabelEmbed.Matr);
        yint depth = YSize(LayerArr);
        p->LayerArr.resize(depth);
        for (yint d = 0; d < depth; ++d) {
            yint count = YSize(LayerArr[d]);
        ...
        FinalLayer->GetDeltaData(&p->FinalLayer.Matr);
    ...
    void StartIteration(float step) override
    {
        MatrixAdd->StartIteration(step);
    }
