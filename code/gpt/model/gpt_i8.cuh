...
__global__ void ConvertHalfToVecFloat(int len, TCuda2DPtr<half> src, float staticScale, float *pSrcScale, TCuda2DPtr<TDst> dst8, float *scaleArr)
{
    ...
    if (t < len) {
        ...
        if (sum2 > 0) {
            ...
            StoreVec<STATE_DIM>(dst8[t], vNorm);
            if (scaleArr && threadIdx.x == 0) {
                if (pSrcScale) {
    ...
    StoreZeroVec<STATE_DIM>(dst8[t]);
    if (scaleArr && threadIdx.x == 0) {
        scaleArr[t] = 0;
    }
}
KERNEL_BLOCK_SIZE(ConvertHalfToVecFloat, WARP_SIZE, SAMPLE_BLOCK);


template <int STATE_DIM, class TElemType>
__global__ void BackpropNormalizeVecs8(int len, TCuda2DPtr<TElemType> srcNorm8, float *srcScale, TCuda2DPtr<half> grad, TCuda2DPtr<half> dst)
{
    int t = blockIdx.x * SAMPLE_BLOCK + threadIdx.y;
    ...
    if (t < len) {
        float v[WCOUNT];
        LoadVec<STATE_DIM>(v, srcNorm8[t]);
        ScaleVec<STATE_DIM>(v, v, VEC_SCALE * srcScale[t]);
        float vGrad[WCOUNT];
...
KERNEL_BLOCK_SIZE(BackpropNormalizeVecs8, WARP_SIZE, SAMPLE_BLOCK);
...
__global__ void MatrixMultI8(TCuda2DPtr<T1> normState8, TCuda2DPtr<T2> matr, TCuda2DPtr<TRes> res, float storeScaleVal, float *storeScalePtr)
{
    ...
    int resStride = res.GetStrideInBytes();
    for (int ty = 0; ty < 2; ++ty) {
        for (int tx = 0; tx < 2; ++tx) {
